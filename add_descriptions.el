...
(defun fill-hash-table (fields)
  ...
  ; base case
  (if (eq (length fields) 0) nil
    (progn
    (puthash (car fields) t final)
...
(defun contains-string (look4 contains)
  ...
  (let (
        (find-index   (string-match-p (regexp-quote look4) contains))
        )
...
(defun find-description-for-key (key descriptions)
  (if (eq (length descriptions) 0) nil
    (if (contains-string key (car descriptions)) (car descriptions)
...
(defun add-hash-value (descriptions ht key)
  (let (
        (matching-description (find-description-for-key key descriptions))
        )
    (if matching-description
        (progn (puthash key matching-description ht) (list key matching-description))
      (list key nil))
  ))
